...
func (h configHandler) Search(bindDN string, searchReq ldap.SearchRequest, conn net.Conn) (result ldap.ServerSearchResult, err error) {
	...
	// there is no need to translate distinguishedName search attribute to dn since
	// all users are returned any way
	//
	// h.cfg.Backend.TranslateDistinguishedNameToDn
